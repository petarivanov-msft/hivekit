 ...
 *   ezb_af_user_cnf_t (status/tsn/dst_addr/src_ep/dst_ep/cluster_id/profile_id)
 *                                             → ezbee/af.h
 ...
 * and one log line — both non-blocking (R4 mitigation).
 *
...
esp_err_t hivekit_report_scd40(const hivekit_scd40_reading_t *reading)
{
     ...
     * (3 per sensor cycle for SCD40 — temp, humidity, CO2). R5: s_tx_queued
     * is bumped after the call returns; a fast local confirm could fire before
     * the increment, causing a momentary confirmed>queued blip — benign for
     * debug-only 5-min heartbeat counters.
...
esp_err_t hivekit_report_sht40(const hivekit_sht40_reading_t *reading)
{
    ...
    /* Send per-attribute ZCL Report Attribute commands with per-command confirm
     * callback. Queued counter increments once per report_attr_cmd_req call
     * (2 per sensor cycle for SHT40 — temp, humidity). */
...
esp_err_t hivekit_report_bme280(const hivekit_bme280_reading_t *reading)
{
    ...
    /* Send per-attribute ZCL Report Attribute commands with per-command confirm
     * callback. Queued counter increments once per report_attr_cmd_req call
     * (3 per sensor cycle for BME280 — temp, humidity, pressure). */
