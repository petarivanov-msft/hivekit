 ...
 *
 * API verified against espressif/button v4.1.6 (latest, master branch):
 *   iot_button_new_gpio_device()  → button_gpio.h
 *     args: (button_config_t*, button_gpio_config_t*, button_handle_t*)
 *   iot_button_register_cb()      → iot_button.h
 *     args: (handle, event, event_args*, cb, usr_data)  ← 5 args
 *   button_config_t               → button_types.h  {long_press_time, short_press_time}
...
{
    /* button v4.1.6: use iot_button_new_gpio_device() with separate gpio config.
     * button_config_t only has {long_press_time, short_press_time}.
    ...
    /* 5-arg register_cb: (handle, event, event_args*, cb, usr_data)
     * event_args=NULL for simple events (SINGLE_CLICK has no extra args) */
    ESP_RETURN_ON_ERROR(
        iot_button_register_cb(btn, BUTTON_SINGLE_CLICK, NULL, on_single_click, NULL),
        TAG, "Failed to register single-click callback");

    /* 3s long press via BUTTON_LONG_PRESS_START with press_time event_arg */
